...
@Test @MainActor func loadsAValidEmptyComposition() {
    let handle = playerCreate()
    let json = #"{"width":1080,"height":1920,"frameRate":{"value":30,"timescale":1},"clips":[],"captions":[],"transitions":[]}"#
    #expect(json.withCString { playerLoadComposition(handle, $0) })
    #expect(playerCurrentTime(handle, 600) == 0)
    #expect(playerRate(handle) == 0)
    playerRelease(handle)
}

@Test @MainActor func composesAndExportsFixtureVideo() async throws {
    ...
    #expect(success)
    #expect((try? output.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0) ?? 0 > 0)
}
...
@Test func roundTripsSecurityScopedBookmark() throws {
    ...
    #expect(resolved.map { URL(fileURLWithPath: String(cString: $0)).resolvingSymlinksInPath().path } == expected)
